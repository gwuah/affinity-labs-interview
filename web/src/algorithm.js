...
function arranger(data) {
  ...
  let currentTime = "09:00AM";
  trackDb[currentTrackNumber] = createTrack();
  while (localCounter < totalHours) {
    for (let min of commonMins) {
      for (let talk of sortedByTime[min]) {
        currentTime = timeParser(currentTime, min);
        trackDb[currentTrackNumber].morning.push({
          talk,
          time: currentTime
        });
        localCounter += min;
        // const nextCounterValue = localCounter + min;
        // if (nextCounterValue < 120) {
